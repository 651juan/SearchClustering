...
if ~isequal(handles.FileName, 0)
    ...
    sim = SimGraph_NearestNeighbors(handles.Data,15,1,1);
    [A,B,C] = SpectralClustering(sim, ...
    handles.NumberOfClusters, ...
    handles.ClusterType);
    indVector = convertClusterVector(A);
    for j  = 1:size(indVector, 1)
       resultString = [resultString char(10) int2str(i) '.' int2str(indVector(j)) char(9) int2str(i) '.' int2str(j)];
    end
   % csvwrite(strcat(int2str(i),'clustered.csv'),indVector);
end
